...
public class BBVAPaymentWebPlugin extends AbstractPaymentWeb {
	...
	public static final String BBVA_PAYMENTWEB_BASE_PROPERTY = PAYMENTWEB_BASE_PROPERTY + "bbva.";
...
//--------------------------------------------------------------------------
	...
	protected void commonFinalNotificacio(HttpServletRequest request, HttpServletResponse response, Long paymentID)
		...
		if (status.getStatus() != PaymentStatus.STATUS_INPROGRESS) {
			...
			log.info(" XYZ IIINNNFFFOOO  Ja s'ha assignat valor de Status pel paymentID = " + paymentID + ": "
					+ status.getStatus());
		...
		String[] all = new String[] { "Ds_Date", "Ds_Hour", "Ds_Amount", "Ds_Currency", "Ds_Order", "Ds_MerchantCode",
				"Ds_Terminal", "Ds_Response", "Ds_MerchantData", "Ds_SecurePayment", "Ds_TransactionType",
				"Ds_Card_Country", "Ds_AuthorisationCode", "Ds_ConsumerLanguage", "Ds_Card_Type" };
...
//          } catch(Exception e) {  }
	...
	protected void startGet(HttpServletRequest request, HttpServletResponse response, String absoluteAddress,
		...
		System.out.println(
				"apiMacSha256.setParameter(\"DS_MERCHANT_MERCHANTNAME\"," + paymentInfo.getSellerName() + ");");
		System.out.println("apiMacSha256.setParameter(\"DS_MERCHANT_PRODUCTDESCRIPTION\","
				+ paymentInfo.getDescriptionProduct() + ");");
		...
		/**
		 * XYZ Opcional: el Valor 0, indicará que no se ha determinado el idioma del
		 * cliente (opcional). Otros valores posibles son: Castellano-001, Inglés-002,
		 * Catalán-003, Francés-004, Alemán-005, Holandés-006, Italiano-007, Sueco-008,
		 * Portugués-009, Valenciano-010, Polaco-011, Gallego-012 y Euskera-013.
		 */
		...
		String html = "<!DOCTYPE html>\n" + "<html>\n" + "<body>\n" + " <form id=\"formPagament\"  action=\""
				+ getPropertyRequired(URL) + "\" method=\"POST\" >\n"
				// ------------
