 ...
 *
 */
public abstract class AbstractPaymentWeb extends AbstractPluginProperties implements IPaymentWebPlugin {
			...
			for (Long id : ids) {
				...
				if (now > ssf.getPaymentInfo().getExpireDate().getTime()) {
					...
					log.info("Passarel·la De Firma: Tancant SignatureSET amb ID = " + id
							+ " a causa de que està caducat " + "( ARA: " + sdf.format(new Date(now)) + " | CADUCITAT: "
